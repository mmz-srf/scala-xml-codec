...
private[xml] sealed trait CardinalityDecoder[F[_], Cy[_], X, A] {
  ...
  //TODO Matthias: Why name? Noone uses it
...
private[xml] object CardinalityDecoder {
  ...
  def list[F[_]:Applicative, I, X, A]: CardinalityDecoder[F, List, X, A] =
    new CardinalityDecoder[F, List, X, A] {
      override def decode(name: String, dec: X => Result[F, A], xs: List[X]): Result[F, List[A]] =
        decodeTraverse(name, dec, xs)
    }

  def nel[F[_]:Applicative, I, X, A]: CardinalityDecoder[F, NonEmptyList, X, A] =
  ...
  F[_]: Applicative,
  G[_]: Traverse, I, X, A](name: String,
                           dec: X => Result[F, A],
                           xs: G[X]): Result[F, G[A]] = {
