...
class CodecDsl[F[_]:Monad] extends EnsureOps {
  ...
  def one[S, X, A](name: String, codec: XmlCodec[F, X, A])
                  (implicit
                   dfe: DecodeFromElem[F, Id, X],
                   append: AppendToElem[X]): XmlCodec[F, ElemValue, A] =
    XmlCodec.collection[F, Id, X, A](name, codec) ~ Codec.fromFunctions(a => a: A, identity)

  def optional[S, X, A](name: String, codec: XmlCodec[F, X, A])
                       (implicit
                        dfe: DecodeFromElem[F, Option, X],
                        append: AppendToElem[X]): XmlCodec[F, ElemValue, Option[A]] =
    XmlCodec.collection(name, codec)

  def zeroOrMore[S, X, A](name: String, codec: XmlCodec[F, X, A])
                         (implicit
                          dfe: DecodeFromElem[F, List, X],
                          append: AppendToElem[X]): XmlCodec[F, ElemValue, List[A]] =
    XmlCodec.collection(name, codec)

  def oneOrMore[S, X, A](name: String, codec: XmlCodec[F, X, A])
                        (implicit
                         dfe: DecodeFromElem[F, NonEmptyList, X],
                         append: AppendToElem[X]): XmlCodec[F, ElemValue, NonEmptyList[A]] =
    XmlCodec.collection(name, codec)

  def attr: XmlCodec[F, AttrValue, String] =
    XmlCodec.attr
