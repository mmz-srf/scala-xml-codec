...
class EncoderDsl[F[_]:Monad] extends EnsureOps {
  ...
  def one[S, X, A](name: String, encoder: XmlEncoder[F, X, A])
                  (implicit
                   append: AppendToElem[X]): XmlEncoder[F, ElemValue, A] =
    XmlEncoder.collection[F, Id, X, A](name, encoder)

  def optional[S, X, A](name: String, encoder: XmlEncoder[F, X, A])
                       (implicit
                        append: AppendToElem[X]): XmlEncoder[F, ElemValue, Option[A]] =
    XmlEncoder.collection(name, encoder)

  def zeroOrMore[S, X, A](name: String, encoder: XmlEncoder[F, X, A])
                         (implicit
                          append: AppendToElem[X]): XmlEncoder[F, ElemValue, List[A]] =
    XmlEncoder.collection(name, encoder)

  def oneOrMore[S, X, A](name: String, encoder: XmlEncoder[F, X, A])
                        (implicit
                         append: AppendToElem[X]): XmlEncoder[F, ElemValue, NonEmptyList[A]] =
    XmlEncoder.collection(name, encoder)

  def attr: XmlEncoder[F, AttrValue, String] =
    XmlEncoder.attr

  def text: XmlEncoder[F, TextValue, String] =
    XmlEncoder.text

  def nonEmptyText: XmlEncoder[F, NonEmptyTextValue, String] =
