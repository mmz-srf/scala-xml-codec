...
private[xml] object GetFromElem {
  ...
  : Result[F, Option[Positioned[String @@ NonEmptyTextValue]]] =
  ...
  implicit def textInstance[F[_]:Monad]: GetFromElem[F, Unit, Id, String @@ TextValue] =
    apply[F, Unit, Id, String @@ TextValue]((elem, _, filter) =>
      flatMapResult(
        Option(Tag.of[TextValue](elem.text)).filterM(filter).map(_.map((_, Option.empty[Int]))))(
  ...
  private def elems[F[_]: Applicative](parent: Elem,
                                       name: String,
                                       filter: Elem => Result[F, Boolean]): Result[F, List[Positioned[Elem]]] = {
  ...
  implicit def elemInstance[F[_]:Monad]: GetFromElem[F, String, Id, Elem] =
    apply[F, String, Id, Elem]((elem, name, filter) =>
      flatMapResult(elems(elem, name, filter)) {
        case h :: Nil => Result.success[F, Positioned[Elem]](h)
        case l => Result
          .error[F, Positioned[Elem]](Path.single(name), s"Exactly one element <$name> expected, found ${l.size}")
      }
    )
  ...
  implicit def elemOptionInstance[F[_]:Monad, CS]: GetFromElem[F, String, Option, Elem] =
    apply((elem, name, filter) =>
      flatMapResult(elems(elem, name, filter)) {
        case Nil => Result.success[F, Option[Positioned[Elem]]](None)
        case List(h) => Result.success[F, Option[Positioned[Elem]]](Some(h))
        case l => Result
          .error[F, Option[Positioned[Elem]]](Path.single(name),
          s"At most one element <$name> expected, found ${l.size}")
      }
    )
